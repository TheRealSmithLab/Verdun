################################################### 7- Exact Fisher tests - Mutation G21641T vs symptoms ###################################################
...
# In this script (7-), we study the mutation G21641T In the previous script (6-), we studied the mutation A24782G.
...
#### Add column mutation_G21641T to joined_df:
# List of the samples with mutation G21641T:
list_samples_G21641T <- c("AB9", "AG8", "AH7", "AI3", "AO1", "AO2", "AP1")
# Create column vectors:
mutation_G21641T <- as.character(rep(c(0), length(joined_df$Sample.ID)))
for (i in 1:length(joined_df$Sample.ID)){
  if (joined_df$Sample.ID[i] %in% list_samples_G21641T) {
    mutation_G21641T[i] = "Yes"
  } 
}
# Add column "mutation_G21641T" to joined_df:
joined_df <- cbind(joined_df, mutation_G21641T)

#### Exact Fisher tests - mutation G21641T vs symptoms:

# Save the results of the statistical tests in a file.txt:
cat("Results exact Fisher tests - mutation G21641T vs symptoms", file = "7-results_exact_fisher_mutation_G21641T.txt")
# add 2 newlines
cat("\n\n", file = "7-results_exact_fisher_mutation_G21641T.txt", append = TRUE)

# For each variable to test:
for (i in col_idx){
  # Create test name:
  test_name <- paste("Check independence Presence of the mutation G21641T /", variables_to_test[j], sep="") 
  # Delete rows where current variable to test == NA:
  reduced_joined_df <- joined_df[!is.na(joined_df[i]), ]
  # Create the contingency table:
  CT <- table(as.factor(reduced_joined_df$mutation_G21641T), as.factor(reduced_joined_df[,i]))
  # Exact Fisher test:
  test <- fisher.test(CT) 
  # Export test outputs:
  cat(test_name, "\n", file = "7-results_exact_fisher_mutation_G21641T.txt", append = TRUE)
  capture.output(CT, file = "7-results_exact_fisher_mutation_G21641T.txt", append = TRUE)
  capture.output(test, file = "7-results_exact_fisher_mutation_G21641T.txt", append = TRUE)
  # Add 2 newlines:
  cat("\n --------------------------------------------------------------------------------- \n", file = "7-results_exact_fisher_mutation_G21641T.txt", append = TRUE)
...
# End of "7-results_exact_fisher_mutation_G21641T.txt" :
cat("END", file = "7-results_exact_fisher_mutation_G21641T.txt", append = TRUE)
